...
export default class Survey extends Component {
  ...
  handleHealth = (name) => {
    this.setState((prevState) => {
      const value = !prevState.healthPreferences[name];
  ...
  handleCalories = (index) => {
    const selected = index === 1 ? 'custom' : 'rec';
    this.setState({
  ...
  getMealPlan = (e) => {
    ...
    this.setState({ loading: true }, () => {
      getPlan(res).then((data) => {
        const par = { num: this.state.planType, data: data };
  ...
  render() {
    ...
    if (this.state.redirect) {
      return <Redirect to={{ pathname: "/meal-plan", state: { data: this.state.data } }} />;
    }
            ...
            <form>
              <Tabs defaultIndex={0} ref={(component) => { this.tabs = component; }} className="Survey__tabs">
                ...
                <Tab heading="4">
                  ...
                  <CheckboxGroup data={healthSpec} toggleHandler={this.handleHealth} isCheckedState={this.state.healthPreferences} />
                  <div className="Survey__goto">
                    <Button name="back" onClick={this.goTo} className="Survey__goto__button--back">Back</Button>
                    <Button name="next" onClick={this.goTo} className="Survey__goto__button--next">Next</Button>
                  </div>
